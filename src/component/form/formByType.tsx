...
export default function RenderFormByType(props: { formItem: { [k: string]: any }, className?: string, style?: CSSProperties, data?: { [p: string]: any }, disabled?: boolean, readonly?: boolean, customField?: { [p: string]: (methods: UseFormReturn) => ReactNode }, customOptions?: { [p: string]: Array<{ [k: string]: any }> }, onSubmit?: (ev: { [p: string]: any }) => void }) {
    ...
    const getOptions = ({ relatives = [], isWatch = false }: { relatives?: Array<{ [p: string]: any }>, page?: number, isWatch?: boolean }) => {
        relatives.forEach((_rel) => {
            ...
            if (props.customOptions) {
                tmpOptions = props.customOptions[`${_rel.Column}_Options`]
            }
    ...
    return <form className={`col ${props.className ?? ""}`} style={props.style}>
            ...
            switch (props.formItem.Category) {
                case 1:
                    return cols.map((_col) => {
                        if (props.customField?.[_col.Name]) return props.customField?.[_col.Name](methods)
        ...
        {rels.map((_rel: any) => {
            if (props.customField?.[_rel.Column]) return props.customField?.[_rel.Column](methods)
...
export const GetWatchRelValues = (props: { methods: UseFormReturn, watchRel: Array<{ [p: string]: any }>, getOptions: any }) => {
